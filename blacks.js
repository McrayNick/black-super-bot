...
function loadHistory(user) {
  const file = path.join(chatDir, `${user}.json`);
...
// 💾 Save history
function saveHistory(user, data) {
  const file = path.join(chatDir, `${user}.json`);
...
if (!m.isGroup && gptdm === "on") {
  try {
    ...
    // 📡 YOUR ORIGINAL GPT API
    let res = await axios.get(
      `${api}/ai/gpt?q=${encodeURIComponent(memory)}`
    );
