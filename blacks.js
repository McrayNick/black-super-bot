...
case "set": {
    ...
    const settingName = args?.split("=")[0];
    let settingValue = args?.split("=")[1];

    // Settings configuration
    const settingsConfig = {
        autoread: { type: 'toggle', values: ['on', 'off'], icon: '📖', desc: 'Auto read messages' },
        autolike: { type: 'toggle', values: ['on', 'off'], icon: '❤️', desc: 'Auto like status' },
        anticall: { type: 'toggle', values: ['on', 'off'], icon: '📞', desc: 'Block calls' },
        antidelete: { type: 'toggle', values: ['on', 'off'], icon: '🗑️', desc: 'Detect deleted messages' },
        welcomegoodbye: { type: 'toggle', values: ['on', 'off'], icon: '👋', desc: 'Welcome/Goodbye msgs' },
        antilink: { type: 'toggle', values: ['on', 'off'], icon: '🔗', desc: 'Block links' },
        antibot: { type: 'toggle', values: ['on', 'off'], icon: '🤖', desc: 'Anti-bot protection' },
        antitag: { type: 'toggle', values: ['on', 'off'], icon: '🏷️', desc: 'Anti-tag protection' },
        gptdm: { type: 'toggle', values: ['on', 'off'], icon: '🧠', desc: 'GPT in DMs' },
        badword: { type: 'toggle', values: ['on', 'off'], icon: '🔞', desc: 'Filter bad words' },
        wapresence: { type: 'select', values: ['online', 'typing', 'recording'], icon: '📡', desc: 'WhatsApp presence' },
        menutype: { type: 'select', values: ['video', 'image', 'link'], icon: '📺', desc: 'Menu display type' },
        prefix: { type: 'select', values: ['none', '!', '*', '†', '★', '?', '$', '.', '/'], icon: '🔣', desc: 'Command prefix' }
    };

    const allSettings = Object.keys(settingsConfig);

    // Help menu
    if (!args || args === 'help') {
        let fancyHelp = `
╔══════════════════════════════════════════╗
║  ✨ 𝑺𝑬𝑻𝑻𝑰𝑵𝑮𝑺 𝑪𝑶𝑵𝑺𝑶𝑳𝑬 ✨  v2.0            
╠══════════════════════════════════════════╣
...
┌──────────────────────────────────────────┐
│  🎮 𝑻𝑶𝑮𝑮𝑳𝑬 𝑺𝑬𝑻𝑻𝑰𝑵𝑮𝑺  (on / off)         
├──────────────────────────────────────────┤
${Object.entries(settingsConfig).filter(([_, cfg]) => cfg.type === 'toggle').map(([key, cfg]) => {
    const val = global.db?.settings?.[key] || 'off';
    const statusIcon = val === 'on' ? '✅' : '❌';
    return `│  ${cfg.icon} ${key.padEnd(14)} ${statusIcon}  ${val.padEnd(3)}  │  ${cfg.desc}`;
}).join('\n')}
├──────────────────────────────────────────┤
│  🎛️  𝑺𝑬𝑳𝑬𝑪𝑻 𝑺𝑬𝑻𝑻𝑰𝑵𝑮𝑺                    
├──────────────────────────────────────────┤
│  📡 wapresence     ${(global.db?.settings?.wapresence || 'online').padEnd(8)}              
│     └─ online / typing / recording      
│                                          
│  📺 menutype       ${(global.db?.settings?.menutype || 'image').padEnd(8)}                
│     └─ video / image / link             
│                                          
│  🔣 prefix         ${(global.db?.settings?.prefix || 'none').padEnd(8)}                  
│     └─ none / ! / * / † / ★ / ? / $ / . / / 
└──────────────────────────────────────────┘
...
║  › .set prefix !                         
╚══════════════════════════════════════════╝
`;await     await client.sendMessage(m.chat, { text: fancyHelp }, { quoted: m });
        break;
    }

    // Check if setting exists
    if (!allSettings.includes(settingName)) {
        const similar = allSettings.find(s => s.includes(settingName?.slice(0,3))) || '';
        await client.sendMessage(m.chat, { 
            text: `╭────────────────────────────────╮\n│  ❌ 𝑬𝑹𝑹𝑶𝑹: 𝑼𝒏𝒌𝒏𝒐𝒘𝒏 𝑺𝒆𝒕𝒕𝒊𝒏𝒈     │\n├────────────────────────────────┤\n│  › "${settingName}" not found  │\n${similar ? `│  › Did you mean: ${similar}?    │` : '│  › Type .set help for list     │'}\n╰────────────────────────────────╯` 
        }, { quoted: m });
        break;
    }
    ...
    if (!settingValue) {
        const currentValue = global.db?.settings?.[settingName] || (config.type === 'toggle' ? 'off' : config.values[0]);
        const statusEmoji = currentValue === 'on' ? '🟢' : (currentValue === 'off' ? '⚫' : '🔵');
        let viewMsg = `
╔════════════════════════╗
║  🔍 𝑺𝑬𝑻𝑻𝑰𝑵𝑮 𝑫𝑬𝑻𝑨𝑰𝑳𝑺     
╠════════════════════════╣
║  ${config.icon} ${settingName.toUpperCase()}       
║  ────────────────────  
║  📊 Status : ${statusEmoji} ${currentValue}      
║  🏷️  Type   : ${config.type}       
║  📝 Desc   : ${config.desc}  
╠════════════════════════╣
║  🎯 Available Options:  
║  ────────────────────  
║  ${config.values.map(v => v === currentValue ? `➜ ${v} ✓` : `  ${v}`).join('\n  ')}  ║
╠════════════════════════╣
║  💡 Try: .set ${settingName}=value  
╚════════════════════════╝
`;
        await client.sendMessage(m.chat, { text: viewMsg }, { quoted: m });
        break;
    }

    // Validate value
    if (!config.values.includes(settingValue)) {
        await client.sendMessage(m.chat, { 
            text: `╭────────────────────────────────╮\n│  ⚠️  𝑰𝑵𝑽𝑨𝑳𝑰𝑫 𝑽𝑨𝑳𝑼𝑬                 │\n├────────────────────────────────┤\n│  › "${settingValue}" not allowed│\n│  › Accepts: ${config.values.join(' / ')} │\n╰────────────────────────────────╯` 
        }, { quoted: m });
        break;
    }
    ...
    try {
        ...
        const oldValue = global.db.settings[settingName] || (config.type === 'toggle' ? 'off' : config.values[0]);
        global.db.settings[settingName] = settingValue;
        
        const arrowIcon = oldValue !== settingValue ? '🔄' : '✅';
        const newStatusIcon = settingValue === 'on' ? '🟢' : (settingValue === 'off' ? '⚫' : '🔵');
...
║  ${config.icon} ${settingName.toUpperCase()}                         ║
║  ────────────────────────────  
║  ${arrowIcon} ${oldValue.toUpperCase()}  →  ${settingValue.toUpperCase()}  ${newStatusIcon}   ║
╠════════════════════════════════╣
║  📌 ${config.desc}     
╚════════════════════════════════╝
`;
